...
def get_template_dir_path():
    ...
    try:
        import settings # サードパーティをimportする前提を考慮した練習記述。オリジナルのセッティングは書き換えてはいけない為
        if settings.TEMPLATE_PATH:
            template_dir_path = settings.TEMPLATE_PATH
    except ImportError:
        pass

    if not template_dir_path: # 今回はsetting.pyを用意していないので必ずこちらが読み込まれる
        # 実行中ファイルのパスを取得し、最終はこのプログラムが存在するパス（roboter）を取得、base_pathとする
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        # os.path.join：パスを結合する = templates が存在するパスを取得
